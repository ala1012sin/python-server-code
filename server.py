...
def trilateration_xy(wifi_list, ap_nodes):
    ...
    def loss(pos):
        return sum((math.sqrt((pos[0] - px)**2 + (pos[1] - py)**2) - d)**2 for (px, py), d in zip(points, distances))
...
def find_closest_node(pos, graph, floor):
    ...
    nodes_in_floor = {
        node: data for node, data in graph.items()
        if z_min <= data["pos"][2] < z_max
    }
...
async def locate_user(data: WiFiRequest):
    floor = estimate_floor_majority(data.apList, ap_nodes)
    if floor == -1:
        return {"error": "층수 추정 실패"}

    xy = trilateration_xy(data.apList, ap_nodes)
    if not xy:
